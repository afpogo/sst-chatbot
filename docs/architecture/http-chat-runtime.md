...
El servicio expone `POST /internal/v1/chat/turns` sólo para `sst-bend`. La llamada exige una credencial de servicio en memoria y devuelve NDJSON ordenado con eventos `delta`, `completed` o `error`.
...
```powershell
$env:PYTHONPATH='src'
$env:CHAT_M2M_TOKEN='local-chat-m2m'
